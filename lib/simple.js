...
END:VCARD`.trim()
    ...
    */
    ...
    sendInter: {
        /**
         * Send InteractiveMessage kreate by Haliq0841 for Chitanda bot
         * @param {Object} media
         * @param {String} body , footer, header, text, subtitle
         * @param {Boolean} hasMediaAttachment
         * @param {String} header
         * @param {Object} cards
         * @param {Object} buttons
         * @param {Boolean} createonly (jika true maka pesan tidak di kirim dan hanya akan membuat interactive message)
         * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted
         */
        async value(jid, {
            media,
            ...
            let mediadoku = ''
            if (media) {
                medok = await generateWAMessageContent(media, {
upload: conn.waUploadToServer
})
                medoktype = (Object.entries(JSON.parse(JSON.stringify(medok, undefined, 2))).map(([nama, isi]) => { return { nama, ...isi } }))[0].nama
                mediadoku = medok[medoktype]
            }
            let natiflow = null
            if (buttons) {
                natiflow = proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                    buttons: buttons,
                })
            }
            let carousel = null
            if (cards) {
                carousel = proto.Message.InteractiveMessage.CarouselMessage.fromObject({
                    cards: cards,
                })
            }
            const returneca = {
                ...
                nativeFlowMessage: natiflow
            }
            if (createonly) return returneca
/*
            if (createonly) return proto.Message.InteractiveMessage.fromObject({
                body: proto.Message.InteractiveMessage.Body.fromObject({
                    text: body
                            }),
                footer: proto.Message.InteractiveMessage.Footer.fromObject({
            ...
            }) //berhenti di sini jika createonly true
*/
